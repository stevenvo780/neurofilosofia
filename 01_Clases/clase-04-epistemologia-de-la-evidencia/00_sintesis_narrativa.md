...
```mermaid
flowchart TD
    Hipotesis[Hipótesis cognitiva] --> Diseño[Diseño experimental<br/>condición + control]
    Diseño --> Tecnica{Técnica}
    Tecnica --> L[Lesión]
    Tecnica --> R[Registro unicelular]
    Tecnica --> N[Neuroimagen<br/>PET/fMRI]
    Tecnica --> E[EEG/MEG]
    Tecnica --> S[Estimulación<br/>TMS/optogenética]
    L --> Datos[Datos]
    R --> Datos
    N --> Datos
    E --> Datos
    S --> Datos
    Datos --> Stat[Análisis estadístico<br/>sustracción]
    Stat --> Inf[Inferencia]
    Inf -.riesgo.-> Art[Artefacto]
    Inf -.riesgo.-> InvInf[Inferencia inversa]
    Inf -.válida si.-> Conv[Convergencia<br/>multitécnica]
    Conv --> Mec[Modelo mecanicista<br/>partes + operaciones + organización]
    Mec --> Expl[Explicación robusta]
```
